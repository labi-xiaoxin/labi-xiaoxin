...
不论并发或是非并发的情况下，在特定的场景下都会发生`数据库与缓存不一致`的问题，因此需要对这样的问题进行相应的分析并解决。

为了保证数据库和缓存的一致性，业内有3种常见的方案：

- 方案一：**数据库先更新，再删除缓存**。（适用并发度不高）
- 方案二：延迟双删：**缓存先删除，再更新数据库，再删除缓存**。（适用高并发）
- 方案三：cache-aside：**数据库先更新，再基于binlog监听进行缓存删除**。

### 9.10 Redis除了做缓存，还能做什么

Redis主要功能就是作为**缓存**使用，但还有以下的功能：

- 消息队列：Redis支持发布/订阅模式和Stream。（不推荐，可靠性低）
- 排行榜：Redis的ZSet实现排行榜功能。（推荐）
- 分布式锁：Redis的单线程能够保证：多个客户端对同一把锁的操作是原子性。（推荐）

### 9.11 Redis如何实现分布式锁

Redis实现分布式锁的核心就是：**Redis单线程的特性**，多个客户端通过**SETNX**命令尝试获取锁，获取成功返回1，并设置过期时间，否则获取失败。直到需要释放锁时，通过**DEL**命令释放。

**需要更完善的使用Redis分布式锁，可以采用Redisson**。


## 十、分布式相关

### 10.1 什么是分布式？什么是微服务？什么是集群？

分布式：**多台服务器中部署不同的服务模块，通过RPC进行协同工作**。

微服务：**将系统拆分为多个服务，每个服务提供不同的功能，通过RPC、MQ、HTTP等进行协同工作**。

集群：**多台服务器中部署相同的服务模块，构成一个集群，通过负载均衡提供服务**。

### 10.2 分布式系统中的CAP理论

CAP指：

- C：Consistency 一致性，**每次读取都会收到最新的写入数据或错误数据**。
- A：Availablility 可用性，**每个请求都会收到响应，但不能保证响应包含最新的写入数据**。
- P：Partition tolerance 分区容错性，**网络节点之间会有消息的丢失，但系统任能够继续运行**。

在CAP理论中，定义了分布式系统**最多只能满足其中两项**。

常见的Nacos支持AP与CP两种模式，Redis为AP模式。**其实大部分设计都为AP模式，只需要确保C为最终一致性，就能够满足大部分系统的需求。**

### 10.3 CAP理论的延伸BASE理论

BASE指：

- BA：Basically Available 基本可用，**在分布式系统出现故障的时候，核心功能依旧可用，部分功能降级**。
- S：Soft State 软状态，**指系统存在中间状态，这个状态不会影响系统整体可用性**。
- E：Eventual Consistency 最终一致性，**指分布式系统中所有数据在经过一段时间后，最终能够保持数据一致的状态**。

### 10.4 分布式锁的实现方式

#### 10.4.1 基于数据库实现分布式锁

- 方案一：创建一张**锁表**，通过**操作表中的数据实现分布式锁的功能，加锁则增加一条记录，释放锁则删除该条记录**。
- 方案二：创建一张**锁表**，通过**数据库的排它锁实现分布式锁的功能，加锁使用for update查询记录，释放锁则为事务提交。**

能够明显感知到，基于数据库实现分布式锁的便捷，**容易理解**，缺点同样很明显：**数据库的性能问题、数据库单点问题、MySQL的优化导致全表扫描**等等。

#### 10.4.2 基于缓存实现分布式锁

详情查看`9.11`，介绍了Redis实现分布式锁。

### 10.5 分布式事务

在分布式系统中，涉及到多个数据库或多个服务之间的事务操作，产生了分布式事务的问题，由此出现了一些分布式事务的解决方案：二阶段提交、三阶段提交、TCC事务、最大努力通知等解决方案。

#### 10.5.1 强一致性下的分布式事务

实现强一致性，则需要一个**协调者**，通过协调者进行协调所有参与者。实现强一致性的方案有：二阶段提交、三阶段提交。

- 二阶段提交：第一阶段：**准备阶段（参与者投票）**，第二阶段：**提交阶段（协调者决定提交或中止）**
- 三阶段提交：由于二阶段提交存在**较多问题**，因此在二阶段的基础上，在第一阶段和第二阶段中间增加了：**预提交阶段**。

#### 10.5.2 最终一致性下的分布式事务

常见方案：本地消息表、最大努力通知、TCC。

##### 10.5.2.1 RocketMQ实现本地消息表

实现步骤如下：

- 1、 发送半消息（half message）：服务向RocketMQ Broker发送一条half message，Broker接收后标记消息为`prepared`状态
- 2、 执行本地事务：服务执行本地事务，本地事务执行成功，通知RocketMQ Broker提交刚刚的事务消息；否则通知RocketMQ Broker回滚刚刚的事务消息。
- 3、 提交事务消息：RocketMQ接收到服务的提交通知后，将消息从`prepared`转为`committed`，此时，该事务消息能够被消费者消费。
- 4、 回滚事务消息：RocketMQ接收到服务的回滚通知后，将消息从`prepared`转为`rollback`，此时，该事务消息被删除，无法被消费者消费。

##### 10.5.2.2 TCC

TCC：Try-Confirm-Cancel。采用了基于业务逻辑的补偿机制，**将整个分布式事务分解为若干子事务**，每个子事务都有`try-confirm-cancel`。

实现步骤如下：

- 1、 Try:**参与者尝试执行本地事务，并对全局事务预留资源**。如果执行成功，参与者返回成功标识，否则返回失败标识。
- 2、 Confirm:所有参与者都返回成功标识，则协调者通知所有参与者提交事务；**参与者提交本地事务，释放全局事务资源**。
- 3、 Cancel:若有参与者返回失败标识，则协调者通知所有参与者回滚事务。

### 10.6 分布式ID

- UUID：32位16进制数字，性能较高，不依赖网络。不适合范围查询、查询效率低。
- 数据库自增：适合单库单表，会有单点故障问题。
- 号段模式：在数据库自增的基础上，取出指定号段，放在缓存中，下一次需要ID从缓存中取，取完再去数据库获取一批新的。
- Redis：依赖Redis的incr命令实现ID自增。
- 雪花算法
